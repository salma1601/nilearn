...
import glob
niimags =  sorted(glob.glob('/volatile/new/salma/atlases/dorsal_DMN/*/*.nii') +\
    glob.glob('/volatile/new/salma/atlases/ventral_DMN/*/*.nii'),
    key=numericalSort)
...
mem = joblib.Memory("/home/salma/CODE/Parietal/nilearn/joblib/nilearn/"\
    "adhd/filtering")
mem2 = joblib.Memory("/home/salma/CODE/Parietal/nilearn/joblib/nilearn/"\
    "adhd/filtering/dmn")
...
if False:
    for subject_n in range(n_subjects):
        ...
        low_pass = .08
        high_pass = .009
        masker1 = nilearn.input_data.NiftiMapsMasker(
            niimags, resampling_target="maps", detrend=True,
        ...
        masker1 = nilearn.input_data.NiftiMapsMasker(
            ...
            low_pass=low_pass, high_pass=high_pass, t_r=t_r, standardize=False,
            memory=mem, memory_level=1, verbose=1)
        masker2 = nilearn.input_data.NiftiMapsMasker(
            niimags, resampling_target="maps", detrend=True,
            low_pass=low_pass, high_pass=high_pass, t_r=t_r, standardize=False,
            memory=mem2, memory_level=1, verbose=1)
        if subject_n == 8:
            region_ts = masker2.fit_transform(filename,
                                             confounds=[hv_confounds, confound_file])
        else:
            region_ts = masker1.fit_transform(filename,
                                             confounds=[hv_confounds, confound_file])
...
subjects = np.load('/home/salma/CODE/Parietal/nilearn/signal_dmn.npy')
for subject_n, subject in enumerate(subjects):
    if subject_n < 8:
        kki.append(subject)
    elif subject_n < 16:
        neuro.append(subject)
    elif subject_n < 24:
        nyu.append(subject)
    elif subject_n < 32:
        ohsu.append(subject)
    else:
        peking.append(subject)
...
covariances = all_matrices2[0]
precisions = all_matrices2[1]
...
for coefs, measure in zip([covariances, precisions, tangents, Z_correlations, Z_partials],
                               measures):
    # Predict ADHD
    u = [[5 * adhd_id, site_id] for (adhd_id, site_id) in zip(adhd, sites)]
    y = LabelBinarizer().fit_transform(u)
    y = adhd[:n_subjects]
    skf = StratifiedKFold(adhd[:n_subjects])
    striate = [2]
    dmn = [3, 4, 5, 6]
