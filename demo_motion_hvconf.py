...
mean_motion = []
for n_subject in range(40):  # range(16, 24)
...
# Selecting a high moving subject
n_subject = np.argsort(mean_motion)[-1]
print n_subject
...
voxels_ts = mem.cache(signal.clean)(voxels_ts, detrend=False,
                                    standardize=False,
                                    confounds=motion)
cleaned_confounds = scaled_high_variance_confounds(voxels_ts,
                                                   n_confounds=n_confounds,
                                                   detrend=True,
                                                   scaling=False)
masker = input_data.NiftiMapsMasker(atlas["maps"], resampling_target="maps")
region_ts = masker.fit_transform(func_filename)  # raw signal
...
cleaned2 = signal.clean(region_ts, detrend=True,
                        standardize=False,
                        confounds=[motion, cleaned_confounds])

# Plotting the extracted confounds
for n, (confound, cleaned_confound) in enumerate(zip(
                                    confounds.T, cleaned_confounds.T)):
        plt.subplot(n_confounds + 1, 1, n + 1)
        plt.plot(confound, label='no cleaning')
        plt.plot(cleaned_confound, label='cleaning')
plt.legend()
...
for n, (confound, cleaned_confound) in enumerate(zip(
    ...
    fs, Pxx = welch(cleaned_confound, window='hanning', nperseg=64,
                          noverlap=32)
...
plt.plot(cleaned[:, regions], label='cleaned, no previous regression')
plt.plot(cleaned2[:, regions], label='cleaned, previous regression')
...
plt.ylabel('top 3 confounds no previous regression')
...
for n, sig in enumerate([cleaned[:, regions], cleaned2[:, regions]]):
    sig = np.squeeze(sig)
    N = sig.shape[0]
    freqs, d_psd = tsa.periodogram(sig)
    psd = np.ones(freqs.shape)
    dB(psd, psd)
    dB(d_psd, d_psd)
    fig1 = plot_spectral_estimate(freqs, psd, (d_psd,), elabels=("Periodogram",))
    plt.title(str(n))
    # Power spectrum of the signal: Welch
    welch_freqs, welch_psd = tsa.get_spectra(sig,
                                             method=dict(this_method='welch',
                                                         NFFT=N,#64
                                                         n_overlap=0)) #32
    welch_freqs *= (np.pi / welch_freqs.max())
    ...
    fig2 = plot_spectral_estimate(freqs, psd, (welch_psd,), elabels=("Welch",))
    plt.title(str(n))
#    plt.plot(welch_freqs, welch_psd)
plt.show()