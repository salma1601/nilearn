...
"""
n_subjects = 16  # subjects to consider for group-sparse covariance (max: 40)
plotted_subject = 50  # subject to plot
...
for n, (func_filename, confound_filename) in enumerate(zip(func_filenames,
                                            confound_filenames)):
    site = adhd_dataset.phenotypic['site'][n][1:-1]
    if 'Peking' in site:
        site = 'Peking'
    if site == 'NeuroImage':
        t_r = 1.96
...
is_adhd = adhd_dataset.phenotypic['adhd']
...
mean_difference = np.mean(
    correlations[is_adhd == 1] - correlations[is_adhd == 0], axis=0)
plot_matrices(np.mean(correlations[:8], axis=0),
              np.mean(correlations[8:16], axis=0), '1, 2 sites')
plot_matrices(np.mean(correlations[:8], axis=0),
              np.mean(correlations[16:24], axis=0), '1, 3 sites')
plot_matrices(mean_difference * (fdr(pval) < 0.05),
              - np.log10(pval) * (fdr(pval) < 0.05),
