...
#                                     standardize=True)
    ...
    if n == plotted_subject:
        ...
        for c in range(7, 11):
            ...
            residual_variance = np.zeros(np.shape(mean_data))
            residual_variance[non_zero_mask] = np.sum((
                predicted_data[non_zero_mask] - data[non_zero_mask]) ** 2, axis=-1)
            explained_variance = np.sum((predicted_data) ** 2, axis=-1)
            total_variance = np.zeros(np.shape(mean_data))
            total_variance[non_zero_mask] = np.sum((data[non_zero_mask] - \
                np.mean(data, axis=-1)[..., np.newaxis][non_zero_mask]) ** 2, axis=-1)
