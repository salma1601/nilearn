...
#                                     standardize=True)
    ...
    if n == plotted_subject:
        ...
        for c in range(3, 7):
            ...
            zero_mask = np.abs(mean_data) < 0.01
            zero_mask = np.sum(data < 0.001, axis=-1) > .5 * n_samples  # 75% of time points are zero
            non_zero_mask = np.logical_not(zero_mask)
            non_zero_data = data[non_zero_mask]
            data_2d = non_zero_data.reshape(-1, n_samples).transpose()
            ...
            predicted_data = predicted_data.reshape(non_zero_data.shape)
            residual_variance = np.sum((predicted_data - non_zero_data) ** 2,
                                       axis=-1)
            explained_variance = np.sum((predicted_data) ** 2, axis=-1)
            total_variance = np.sum((non_zero_data - np.mean(
                non_zero_data, axis=-1)[..., np.newaxis]) ** 2, axis=-1)
#            total_variance = np.sum((data - np.mean(data, axis=-1)[..., np.newaxis]) ** 2, axis=-1)
            percent_variance = np.zeros(mean_data.shape)
            percent_variance[non_zero_mask] = 100 * (1 - residual_variance / total_variance)
#            percent_variance[non_zero_mask] = 100 * explained_variance / total_variance
            percent_variance[mean_data == 0.] = 0.
#            percent_variance[np.abs(percent_variance) < .5] = 0.
#            percent_variance[percent_variance > 100] = 0.
            percent_variance[percent_variance < 0.] = 0.
            percent_variance = np.sqrt(percent_variance)
