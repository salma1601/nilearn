...
def cheap_mean(mats, init=None, max_iter=10, tol=1e-7):
    a_mean = np.mean(mats, axis=0)
    cheap = a_mean
    step = 1.
    res_old = np.inf
    for n in range(max_iter):
        cheap_inv = linalg.inv(cheap)
        cheap = (1 - step) * cheap + step * a_mean
        res = np.linalg.norm(1.5 * cheap - 2. * a_mean + .5 * np.mean([mat.dot(
            cheap_inv).dot(mat) for mat in mats], axis=0))
        if res < res_old:
            res_old = res
        else:
            res = res_old
            step = .5 * step
        if res / cheap.size < tol:
            break

    if tol is not None and res / cheap.size >= tol:
...
def geometric_mean(mats, init=None, max_iter=10, tol=1e-7):
    ...
    init : numpy.ndarray, shape (n_features, n_features) or None, optional
        ...
        geometric mean of arithmetic and harmonic means of mats if None.
...
#        mats1 = mats[::2]
#        mats2 = mats[1::2]
#        mats2_inv = np.array([np.linalg.inv(mat) for mat in mats2])
#        spd1 = np.mean(mats1, axis=0)
#        spd2 = np.linalg.inv(np.mean(mats2_inv, axis=0))
...
#        geo = spd2_sqrt.dot(map_sym(np.sqrt, spd2_inv_sqrt.dot(spd1).dot(
...
def normalized_mean(mats, init=None, max_iter=10, tol=1e-7):
    if init is None:
        n_mean = np.mean(mats, axis=0)
    n_features = mats[0].shape[0]
    off_diag = n_mean - np.diag(np.diag(n_mean))
    from scipy.linalg import logm
    for n in range(max_iter):
        n_mean_ = np.eye(n_features) + off_diag
        logs = [logm(np.linalg.inv(mat).dot(n_mean)) for mat in mats]
        off_diag_new = np.mean(logs, axis=0)
        diff = np.linalg.norm(off_diag - off_diag_new)
        off_diag = off_diag_new
        if diff < tol:
            break
    n_mean = np.eye(n_features) + off_diag
    return n_mean
...
def grad_geometric_mean(mats, init=None, max_iter=10, tol=1e-7):
    ...
    # Initialization
    if init is None:
        mats_inv = np.array([np.linalg.inv(mat) for mat in mats])
        spd1 = np.mean(mats, axis=0)
        spd2 = np.linalg.inv(np.mean(mats_inv, axis=0))
        vals_spd2, vecs_spd2 = np.linalg.eigh(spd2)
        spd2_sqrt = map_eig(np.sqrt, vals_spd2, vecs_spd2)
        spd2_inv_sqrt = map_eig(np.sqrt, 1. / vals_spd2, vecs_spd2)
        geo = spd2_sqrt.dot(map_sym(np.sqrt, spd2_inv_sqrt.dot(spd1).dot(
...
class CovEmbedding(BaseEstimator, TransformerMixin):
    ...
    def fit(self, X, y=None):
        ...
        if self.kind == 'tangent':
            ...
            self.mean_cov_ = geometric_mean(covs, max_iter=10, tol=1e-7)
