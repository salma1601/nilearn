def random_wishart(scale_mat, dof, size=None, rand_gen=None):
    """Draw random matrix from Wishart distribution.

    Parameters
    ==========
    scale_mat: (M, M) numpy.ndarray
        Scale matrix. Raise an error if not square symmetric positive definite.

    dof: int
        Degrees of freedom. Raise an error if dof < M.

    size: int or tuple of ints, optional
        Given a shape of, for example, (m, n, k), m * n * k samples are
        generated, and packed in an m-by-n-by-k arrangement. Because each
        sample has shaped (M, M), the output shape is (m, n, k, M, M).
        If no shape is specified, a single sample of shape (M, M) is returned.
    ...
    out: numpy.ndarray
        ...
        shape is (M, M). In other words, each entry out[i, j, ..., :] is an
        (M, M) array drawn from the distribution.
    ...
    if size is None or size is ():
        x = rand_gen.multivariate_normal(np.zeros(), scale_mat, dof)
        out = (x.T).dot(x)
    else:
        out = np.empty((size,) + scale_mat.shape)
        if isinstance(size, tuple):
            new_size = size[1:]
        else:
            new_size = None
        for k in range(size):
            out[k] = random_wishart(dof, scale_mat, new_size)

    return out


def sample_wishart(sigma, dof, rand_gen=None):
    """
    Returns a sample from the Wishart distn, conjugate prior for precision
    matrices.
    """
    if rand_gen is None:
        rand_gen = np.random.RandomState(0)
    ...
    if (dof <= 81 + n) and (dof == round(dof)):
        # direct
        X = np.dot(chol, rand_gen.normal(size=(n, dof)))
    else:
        A = np.diag(np.sqrt(rand_gen.chisquare(dof - np.arange(0, n), size=n)))
        A[np.tri(n, k=-1, dtype=bool)] = rand_gen.normal(
            size=(n * (n - 1) / 2.))
        X = np.dot(chol, A)
...
def sample_spd_normal(mean, cov=None, rand_gen=None):
    ...
    Parameters
    ==========
    mean: (M, M) numpy.ndarray
        Mean of the (M, M) matrix distribution. Raise an error if not square
        symmetric positive definite.
    cov: (M * (M + 1) / 2, M * (M + 1) / 2) numpy.ndarray
        Covariance matrix on the manifold. Raise an error if not correct shape
        or not symmetric positive definite.
    ...
    if cov is None:
        ...
        raise ValueError("Shapes of mean and covariance matrices are not"
                         "compatible, expected covariance of shape ({0}, {0})"\
                         .format(p))
...
def random_spd_normal(mean, cov, size):
    """Draw random spd matrix from Gaussian distribution on the manifold.
    ...
    if size is None or size is ():
        tangent = np.random.multivariate_normal(0, cov, size)
        out = sqrtm(mean).dot(expm(tangent)).dot(sqrtm(mean))
    else:
        out = np.empty((size,) + mean.shape)
        if isinstance(size, tuple):
            new_size = size[1:]
        else:
            new_size = None
        for k in xrange(size):
            out[k] = random_spd_normal(mean, cov, new_size)
