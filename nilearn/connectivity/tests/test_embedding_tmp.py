...
def test_geometric_mean_properties():
    ...
    for p in [0., .5, 1.]:  # proportion of badly conditionned matrices
        ...
        from nilearn.connectivity.embedding import cov_to_corr, normalized_mean        
        corrs = [cov_to_corr(spd) for spd in spds]
        n_mean = normalized_mean(corrs)
        print n_mean.shape
        assert_array_almost_equal(np.ones(n_features), np.diag(n_mean))
        chek_mat('spd', n_mean)
        
#        print np.mean(sparse)
#        init = .5 * np.mean(spds,axis=0) + .5 * linalg.inv(np.mean([linalg.inv(mat) for mat in spds],axis=0))
#        geo = geometric_mean(spds, max_iter=max_iter, tol=1e-5, init=init)
    print ok
...
def cheap_mean():
    ...
    for p in [.5, 1.]:  # proportion of badly conditionned matrices
        ...
        if p < 1:
            ...
            max_iter = 60
        cheap = cheap_mean(spds, max_iter=max_iter, tol=1e-5)
...
def test_vec_to_sym():
    ...
    # Check error if unsuitable sizen
    vec = np.ones(31)
