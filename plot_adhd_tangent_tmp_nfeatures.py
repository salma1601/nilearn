...
i = 0
max_selected = 200
z_lda = np.zeros((5, max_selected))
z_qda = np.zeros((5, max_selected))
z_log = np.zeros((5, max_selected))
n_used = n_subjects
...
#    scores_log = []
#    scores_svm = []
    for n_selected in range(2, max_selected + 2, 10):
#        cv = n_used / 2
        X = coefs.copy()#[:, 3:7, 3:7]
...
#            cv_scores_svm.append(clf_svm.score(X_test, y_test))
            try:
                ...
                cv_scores_qda.append(0.)
                pass
        print(" {0} for {1} measure".format(np.mean(cv_scores_lda), measure))
        scores_lda.append(np.mean(cv_scores_lda))
        scores_qda.append(np.mean(cv_scores_qda))
#        scores_log.append(np.mean(cv_scores_log))
#        scores_svm.append(np.mean(cv_scores_svm))
    z_lda[i] = np.array(scores_lda)
    z_qda[i] = np.array(scores_qda)
#    z_log[i] = np.array(scores_log)
#    z_svm[i] = np.array(scores_svm)
    i += 1
plt.figure()
lineObjects = plt.plot(np.arange(2, max_selected + 2), z_lda.T)
plt.legend(iter(lineObjects),('cov', 'prec', 'tang', 'Z-corr', 'Z-part'), loc=4)
plt.title('LDA with {0} subjects, cv={1}'.format(n_used, cv))
plt.xlabel('number of features')
plt.figure()
lineObjects = plt.plot(np.arange(2, max_selected + 2), z_qda.T)
plt.legend(iter(lineObjects),('cov', 'prec', 'tang', 'Z-corr', 'Z-part'), loc=4)
plt.title('QDA with {0} subjects, cv={1}'.format(n_used, cv))
plt.xlabel('number of features')
plt.figure()
lineObjects = plt.plot(np.arange(2, max_selected + 2), z_log.T)
plt.legend(iter(lineObjects),('cov', 'prec', 'tang', 'Z-corr', 'Z-part'), loc=4)
plt.title('Logisitic regression with {0} subjects, cv={1}'.format(n_used, cv))
plt.xlabel('number of features')
plt.show()
