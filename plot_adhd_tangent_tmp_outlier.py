...
for coefs, measure in zip([d for d in dist], measures):
    ...
    for train_index, test_index in skf:
        ...
        y_train, y_test = sites[train_index], sites[test_index]
#        clf_reg = LogisticRegression()
#        clf_reg.fit(X_train, y_train)
#        cv_scores_reg.append(clf_reg.score(X_test, y_test))
#        print("Residual sum of squares: %.2f"
#              % np.mean((clf_reg.predict(X_test) - y_test) ** 2))
#        # Explained variance score: 1 is perfect prediction
#        print('Variance score: %.2f' % clf_reg.score(X_test, y_test))
#    print '--------', measure, ': ', np.mean(cv_scores_reg), '------'
        ...
        cv_scores_log.append(clf_log.score(X_test, y_test))
        cv_scores_ovr.append(clf_svc_ovr.score(X_test, y_test))
    print '=========================================='
    print measure
    # Explained variance score: 1 is perfect prediction
    clf_log = OneVsOneClassifier(LinearSVC(random_state=0))
    score = np.mean(cross_val_score(clf_log, X, y, cv=skf))
    print 'Variance score: ', np.mean(cv_scores_log), '=', score
    clf_svc_ovr = LinearSVC(random_state=0, multi_class='ovr')
    score = np.mean(cross_val_score(clf_svc_ovr, X, y, cv=skf))
    print 'Variance score one vs rest: ', np.mean(cv_scores_ovr), '=', score

#    null_cv_scores = permutation_test_score(clf_svc_ovr, fmri_masked, target, cv=skf)
#    print 'permutation score is', np.mean(null_cv_scores)
#    y_pred = OneVsRestClassifier(LinearSVC(random_state=0)).fit(X, y).predict(X)
#    print y_pred, 'accuracy: ', accuracy_score(y, y_pred)
