...
for measure in measures:
    estimator = {'cov_estimator':MinCovDet(assume_centered=True),
                 'kind': measure}
...
for sub_n in range(len(subjects)):
    ...
    for n, conns in enumerate([covariances, precisions, tangents, Z_correlations, Z_partials]):
        if n == 2:
            dist[n, sub_n] = np.linalg.norm(tangents[sub_n])
        else:
            dist[n, sub_n] = np.linalg.norm(conns[sub_n]- np.mean(conns, axis=0))

# Try also the distance to the median
# Positive results without removing the mean
# Removing the mean : tangent 60% one vs rest
plt.figure()
lineObjects = plt.plot(np.arange(0, sub_n + 1), dist.T - np.ones((n_subjects, 5)) * dist.mean(axis=1))
