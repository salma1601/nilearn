...
zoom = 1.5
axins = inset_axes(ax, .5, zoom, loc=6)
all_positions = np.array(all_positions, dtype=float)
diff = (all_positions[1] - all_positions[0])
all_positions[1] = all_positions[0] + 1. * diff
all_positions[2] = all_positions[1] + 1. * diff
bp = axins.boxplot(data, widths=widths, sym=sym,
                   positions=all_positions)
set_box_colors(bp, colors + colors + colors + colors)

# sub region of the original image
x1, x2, y1, y2 = 0.5, 3.5, .1, .75
