...
for condition in conditions:
    ...
    for connectivity_id, connectivity in enumerate(
        ...
        colors_scatter += [u for conn in 
                   partials[condition][connectivity_id + 1:]]
    my_correlations[condition] = analyzing.compute_pairwise_distances(
        correlations[condition], distance_type='euclidean')
    my_parts[condition] = analyzing.compute_pairwise_distances(
...
plt.scatter(my_correlations[condition],  my_parts[condition], c='r')
