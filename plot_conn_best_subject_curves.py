...
for n_outliers in range(max_outliers):
    ...
    outliers_combinations = [shuffle(
        high_motion_subjects, random_state=1)[:n_outliers] for k in
        range(max_combinations)]

    # Incrementally add high moving subjects
    for n, outliers in enumerate(outliers_combinations):
        if n in indices:
            if np.array(outliers).shape != (0,):
...
plt.xlabel('number of noisy matrices used')
plt.ylabel(r'$\|mean - amean\, of\, non-noisy\, matrices\|$')
