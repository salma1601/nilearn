...
raw_dataset = dataset_loader.load_conn(
    conn_folder_filt, conditions=[condition], standardize=standardize,
    networks=networks, files_pattern='data/ROI_Subject%03d_Session%03d.mat')
raw_subjects = raw_dataset.time_series[condition]
all_confounds = dataset['motion'][condition]
from sklearn import linear_model
from nilearn import signal
explained_variances = []
# Compute variance explained by quadratic terms
for subject, confounds in zip(raw_subjects, all_confounds):
    quadratic_confounds = confounds ** 2
    confounds_to_regress = signal.high_variance_confounds(subject)
    confounds_to_regress = confounds
    regr = linear_model.LinearRegression()
    regr.fit(subject, confounds_to_regress)
    # Explained variance score: 1 is perfect prediction
    explained_variances.append(regr.score(subject, confounds_to_regress))
