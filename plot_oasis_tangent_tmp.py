...
for coefs, measure in zip([covariances, precisions, tangents, Z_correlations, Z_partials], measures):
    scores = []
    scores_qda = []
    for n_used in range(30, n_subjects + 1):
#        cv = n_used / 2
...
#            clf.score(X_test, y_test)
    ...
    i += 1
plt.figure()
lineObjects = plt.plot(np.arange(30, n_subjects + 1), z.T)
...
plt.figure()
lineObjects = plt.plot(np.arange(30, n_subjects + 1), z_qda.T)
plt.legend(iter(lineObjects),('cov', 'prec', 'tang', 'Z-corr', 'Z-part'), loc=4)
plt.title('QDA with {0} regions DMN , cv={1}'.format(n_regions, cv))
plt.xlabel('number of subjects')
plt.show()