...
mem2 = joblib.Memory("/home/salma/CODE/Parietal/nilearn/joblib/nilearn/adhd/filtering/no_mvt")
...
#    t_r = 2.5
    ...
    if subject_n < max_outliers:
        region_ts2 = masker2.fit_transform(filename,
...
for n_out, all_matrices2 in enumerate(noisy_matrices):
    ...
    for n_meas, measure in enumerate(measures):
        for n_site in range(5):
            ...
            filename = os.path.join(
                '/home/salma/slides/Parietal2014/Images/robustness',
                fig_title + ".pdf")
    ...
    for n, conns in enumerate(conns_all):
        ...
        if n_out < 3:
            ...
            filename = os.path.join(
                '/home/salma/slides/NiConnect/Images/robustness',
                fig_title + ".pdf")
            if not os.path.isfile(filename) or overwrite:
                pylab.savefig(filename)
                os.system("pdfcrop %s %s" % (filename, filename))
...
plt.xticks(percent_outliers, percent_str, size=8)
plt.ylabel('relative error')
fig_title = 'froe_relative_err_corr_part_corr-gmean' + estimators[n_estimator][0]
filename = os.path.join(
    '/home/salma/slides/NiConnect/Images/robustness',
    fig_title + ".pdf")
if not os.path.isfile(filename) or overwrite:
    pylab.savefig(filename)
    os.system("pdfcrop %s %s" % (filename, filename))
np.save('/home/salma/CODE/Parietal/nilearn/signals_MSDL', subjects)
#np.save(subjects2, '/home/salma/CODE/Parietal/nilearn/raw_signals_MSDL')